...
/*   Updated: 2015/06/01 17:08:19 by rdantzer         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		ray_cast_wall_dist(t_raycast *r)
{
	...
	if (r->draw_end >= r->h)
		r->draw_end = r->h - 1;
	if (r->side == 1)
		r->wall_x = r->ray_pos.x + ((r->map_y - r->ray_pos.y +
		(1 - r->step_y) / 2) / r->ray_dir.y) * r->ray_dir.x;
	else
		r->wall_x = r->ray_pos.y + ((r->map_x - r->ray_pos.x +
			(1 - r->step_x) / 2) / r->ray_dir.x) * r->ray_dir.y;
}
